let overloadManager = scrmljs.overloadManager = {}, emptyFunction = scrmljs.emptyFunction;
/*
...
overloadManager.protoModel.closeProcess = function closeProcess() {
    ...
    if (!(this.openProcesses >= 0)) throw Error("overload manager has " + this.openProcesses + " open process");
}
...
overloadManager.protoModel.addTicket = function addTicket(item, ticketFunction, ...data) {
    if (typeof this.ticketFunctions[ticketFunction] !== "function") throw Error("do not recognize ticket function " + ticketFunction);
